        ...
        const response = await axios.post<DeepSeekResponse>(
            'https://api.deepseek.com/v1/chat/completions',
            {
                ...
                messages: [
                    {
                        role: 'system',
                        content: '你是一个专业的周报总结助手，请根据提供的 Git 提交记录，生成一份结构清晰、重点突出的周报总结。请使用1.2.3.4的格式列出工作内容，不要包含原始提交记录。'
                    },
                    {
                        role: 'user',
...
// 处理日期选择
async function handleDateSelection(defaultDates: { startDate: string, endDate: string }): Promise<{ startDate: string, endDate: string } | undefined> {
    // 构建快速选择项
    const quickPicks = [
